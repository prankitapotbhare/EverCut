...
**For Customer:**
1. Run `01-authentication` → **Check After OTP** (with `roleType: "CUSTOMER"`)
2. If new user, run **Complete Customer Profile**
3. Now you can use all Customer collections (02-05)

**For Barber:**
1. Run `01-authentication` → **Check After OTP** (with `roleType: "BARBER"`)
...
```
01-Authentication
  ↓ Check After OTP → Complete Customer Profile
02-Customer Profile
...
```
01-Authentication
  ↓ Check After OTP → Complete Barber Profile
...
```http
POST {{base_url}}{{api_prefix}}/auth/complete-profile/customer
